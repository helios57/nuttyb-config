...
function gadget:Initialize()
    ...
    for _, name in pairs(baseNames) do
        ...
        if u1 and u2 then mergeMap[u1.id] = u2.id end

        -- T2 -> T3, T3 -> T4, T4 -> T5
        for i = 2, 4 do
            local cur = UnitDefNames[name .. "_t" .. i]
            local next = UnitDefNames[name .. "_t" .. (i+1)]
            if cur and next then mergeMap[cur.id] = next.id end
        end
    end
end
...
function gadget:UnitFinished(unitID, unitDefID, teamID)
    ...
    local function ExecuteMerge(uTL, uTR, uBL, uBR)
        if uTL and uTR and uBL and uBR then
            ...
            local h1, _, _, _, p1 = spGetUnitHealth(uTL)
            local h2, _, _, _, p2 = spGetUnitHealth(uTR)
            local h3, _, _, _, p3 = spGetUnitHealth(uBL)
            local h4, _, _, _, p4 = spGetUnitHealth(uBR)
            ...
            if newUnit then
                -- Transfer relative health percentage? Or sum health?
                -- If we sum, it might exceed max.
                -- For now, let's just assume it spawns with full health or proportional.
                -- Let's give it the average build progress? (They should be finished though)

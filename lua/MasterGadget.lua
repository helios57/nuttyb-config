...
local fusionBuckets = {}
for i = 0, BUCKET_COUNT - 1 do fusionBuckets[i] = {} end
...
local function ProcessFusion(unitID)
    ...
    local nearby = spGetUnitsInCylinder(x, z, searchRadius, spGetUnitTeam(unitID)) -- Use unit's team
    ...
    local function ExecuteMerge(uTL, uTR, uBL, uBR)
        if uTL and uTR and uBL and uBR then
            ...
            local teamID = spGetUnitTeam(uTL)
...
fusioncore_UnitFinished = function(unitID, unitDefID, teamID)
    -- Early filter to avoid queuing everything
    local minTier = tonumber(modOptions.fusion_mintier) or 1
    local currentTier = unitTier[unitDefID] or 0
    if currentTier < minTier then return end
    ...
    -- Queue into bucket
    local bucket = unitID % BUCKET_COUNT
    table.insert(fusionBuckets[bucket], unitID)
end

fusioncore_GameFrame = function(n)
    local bucketIndex = n % BUCKET_COUNT
    local bucket = fusionBuckets[bucketIndex]

    if #bucket > 0 then
        -- Process and Clear
        for i = 1, #bucket do
            ProcessFusion(bucket[i])
            bucket[i] = nil
        end
        -- Since we nil'd explicitly, the table length is still same but contents nil?
        -- No, table_insert appends. Niling entries leaves holes.
        -- Better to re-init the table to keep it clean.
        fusionBuckets[bucketIndex] = {}
    end
end
...
function gadget:GameFrame(...)
  ...
  if fusioncore_GameFrame then fusioncore_GameFrame(...) end
